...
class Astar2D:
    ...
    def __compute_cost(start_node, end_node, current_node):
        ...
        current_node.G = np.linalg.norm(
            np.array(start_node.pos) - np.array(current_node.pos))
        current_node.H = np.linalg.norm(
            np.array(end_node.pos) - np.array(current_node.pos))
...
class Astar3D(Astar2D):
    ...
    def get_neighbors(self, node):
        ...
        for md in self.moving_dirs:
            ...
            if 0 <= n[0] < self.gw and 0 <= n[1] < self.gh and 0 <= n[
                2] < self.gd and (self.map[xmask].any()):
                ret.append(n)
        return ret
